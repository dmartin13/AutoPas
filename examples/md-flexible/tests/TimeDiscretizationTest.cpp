...
TEST_F(TimeDiscretizationTest, testCalculateVelocities) {
  ...
  // First timestep
  TimeDiscretization::calculateVelocities(*autoPas, *PPL, 0.1, false, -1, false);
...
#endif
  EXPECT_NO_THROW(TimeDiscretization::calculatePositionsAndResetForces(*autoPas, *PPL, deltaT, {0., 0., 0.}, true, false))
      << "Updating the position of a slow particle should not throw an exception.";
  // fast particle -> exception
  autoPas->begin()->setV({1., 0., 0.});
  EXPECT_THROW(TimeDiscretization::calculatePositionsAndResetForces(*autoPas, *PPL, deltaT, {0., 0., 0.}, true, false),
               std::runtime_error)
